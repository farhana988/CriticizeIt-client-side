...
const UpdateReviewModal = ({ review, isOpen, onClose, setReviews }) => {
  ...
  return (
    <div className="fixed inset-0 bg-gray-500 bg-opacity-50 flex items-center justify-center z-50">
      <div className="bg-white p-6 rounded-lg w-11/12 md:w-9/12 lg:w-7/12 
      h-5/6 md:h-3/5 lg:h-3/6 overflow-y-scroll">
        <h2 className="  text-5xl  lg:text-8xl 
        text-center text-primary active font-semibold mb-6">
          Update Review</h2>
        <form onSubmit={handleSubmit} className="space-y-6">
          <section className="grid gap-5 lg:gap-10 md:grid-cols-2">
            {/* Service Title */}
            <div>
              <label className="label text-gray-900 text-xl font-semibold">
                Service Title</label>
              <input
                type="text"
                name="serviceTitle"
          ...
          {/* Review Text */}
          <div>
            <label className="label text-gray-900 text-lg font-semibold">
              Review Text</label>
            <textarea
              name="reviewText"
